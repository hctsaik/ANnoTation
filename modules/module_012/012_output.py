...
def _render_missing_tool_picker(tool_id: str) -> None:
    ...
    def persist(selected: str) -> None:
        ...
        st.session_state.pop("m012_launch_error", None)
        st.session_state.pop("m012_missing_tool_id", None)
        st.session_state.pop(f"m012_tool_path_panel_{tool_id}", None)
        st.session_state["m012_launch_ok"] = (
            f"已設定 {display_name}：{Path(selected).as_posix()}"
        )
        st.rerun()

    panel_key = f"m012_tool_path_panel_{tool_id}"
    if st.button(
        f"📁 指定 {display_name} 位置",
        key=f"m012_choose_external_tool_{tool_id}",
        help="展開一次性路徑設定；瀏覽器不會上傳執行檔。",
    ):
    ...
    with apply_col:
        apply_clicked = st.button(
            "套用完整路徑", key=f"m012_apply_external_tool_{tool_id}", use_container_width=True,
        )
    with cancel_col:
        if st.button("取消", key=f"m012_cancel_external_tool_{tool_id}", use_container_width=True):
...
[data-testid="stExpander"] { margin-top: 0 !important; margin-bottom: 0.25rem !important; }
    ...
    if _launch_err := st.session_state.get("m012_launch_error"):
        st.error(f"啟動失敗：{_launch_err}")
        if _missing_tool_id := st.session_state.get("m012_missing_tool_id"):
